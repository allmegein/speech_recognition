...
featureMatrix = [melCoeffs frameEnergies]; % feature matrix


%% SINGLE LAYER PERCEPTRON %%

% define initial parameters and train.
ip = featureMatrix; % input of data for training
[R , C] = size(ip);
numIn = C - 1; % number of input
bias = 1; % bias
weights = xlsread('Updated Weights of MFCC.xls'); % call updated weights.
out = zeros(R , 1);
for tr = 1 : R
     y = bias * weights(1,1) + ip(tr,1:end) * weights(2:end);
...
if (mean(out) >= 0.5)
    fprintf('It is a bird sound.\n');
else
    fprintf('It is a guitar sound.\n');
end
